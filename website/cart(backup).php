...
<div class="header">
<div class="title fw-5">Shopping cart <span>(<?php
...
$total = $total; // Example amount
$max = 2000;   // Maximum amount for full width

// Calculate percentage width
$widthPercent = ($total >= $max) ? 100 : ($total / $max) * 100;
?>

     <!-- Optional: Label or value -->
...
Buy <span class="price fw-6">₹2000.00</span> more to enjoy <span class="fw-6">Free
Shipping</span>
</div>
</div>
...
<div class="tf-mini-cart-sroll"><?php
...
<!-- <div class="price fw-6" data-product-id="<?php echo $row['sno']; ?>">
    ₹<?php echo number_format($row['total_price'], 2); ?>
</div>
 -->


<!-- For each product, use a unique ID and store product ID in data attribute -->
<div class="price fw-6 product-price" 
     id="product-price-<?php echo $row['sno']; ?>" 
     data-product-id="<?php echo $row['sno']; ?>">
    ₹<?php echo $row['total_price'] ?? '0'; ?>
</div>
...
function updateAllPrices() {
    ...
    document.querySelectorAll('.product-price').forEach(priceDiv => {
        const productId = priceDiv.dataset.productId;
...
<!-- <div class="price fw-6">Quantity:<?php echo"$row[qty]"; ?></div> -->
<div class="tf-mini-cart-btns">
 ...
 <a href="#" class="remove-item" data-id="<?php echo $row['sno']; ?>" data-product-id="<?php echo $product_id; ?>">
...
<div class="tf-cart-total">Subtotal</div>
    ...
    <!-- <div class="price fw-6" id="total_prices">₹</div> -->
    <div id="totalPrice">Loading...</div>
<script>
function fetchTotalPrice() {
    // Pass PHP variable directly to JavaScript
    const email = '<?php echo isset($ipaddress) ? addslashes($ipaddress) : ""; ?>';
    
    // Validate email/IP address exists
    if (!email) {
    ...
    fetch(`get_discount_price.php?email=${encodeURIComponent(email)}`)
        ...
        .catch(error => {
            ...
            const priceElement = document.getElementById('totalPrice');
            if (priceElement) priceElement.innerText = 'Price update failed';
        });
}

// More reasonable interval (2000ms = 2 seconds)
...
<div class="overplay tf-mini-cart-tool-close"></div>
<form class="tf-product-form-addgift"></form>
<div class="tf-mini-cart-tool-content">
...
function updateProductPrice(productId) {
    ...
    fetch(`get_price.php?id=${productId}`)
        .then(response => {
        ...
        .catch(error => console.error('Error fetching price for product', productId, error));
}

// Initialize price updates for all products
function initPriceUpdates() {
    // Clear any existing intervals
    Object.values(priceIntervals).forEach(interval => clearInterval(interval));
    ...
    document.querySelectorAll('[data-product-id]').forEach(element => {
        ...
        priceIntervals[productId] = setInterval(() => {
...
// Reinitialize if cart contents change (you'll need to call this when adding/removing products)
...
$(document).ready(function() {
  ...
  // Function to update cart totals (subtotal, etc.)
  function updateCartTotals() {
    $.ajax({
      ...
      success: function(response) {
        ...
        $('.tf-totals-total-value').text('₹' + data.subtotal);
        // Update any other totals as needed
      },
      error: function(xhr, status, error) {
        console.error(error);
      }
...
$(document).on('click', '.remove-item', function(e) {
    ...
    var itemElement = $(this).closest('.tf-mini-cart-item'); // The container of the item to remove
        ...
        success: function(response) {
            if(response === 'success') {
                ...
                itemElement.fadeOut(300, function() {
                    ...
                    $('.header .title span').text('(' + (currentCount - 1) + ')');
