...
<body class="preload-wrapper">
    ...
    <div id="wrapper">
        ...
        <!-- grid -->
        <section>
    <div class="page-search-inner" style="padding: 0 25px;">
        <div class="tf-search-head">
            <form class="tf-mini-search-frm" action="search-home.php" method="GET">
                <fieldset class="text">
                    <input type="text" placeholder="Search" class="" name="q" tabindex="0" value="<?php echo isset($_GET['q']) ? htmlspecialchars($_GET['q']) : ''; ?>"
            ...
            <div class="tf-col-quicklink">
                ...
                <a href="shop-collection-list.php">Fashion</a>,
                <a href="shop-men.php">Men</a>,
                <a href="shop-women.php">Women</a>,
                <a href="shop-collection-sub.php">Accessories</a>
            </div>
        </div>
    </div>
    <div class="container">
        ...
        <div class="wrapper-control-shop">
            <div class="meta-filter-shop">
                <div id="product-count-grid" class="count-text">
                    <?php if(isset($_GET['q'])): ?>
                        Showing results for: "<?php echo htmlspecialchars($_GET['q']); ?>"
                    <?php endif; ?>
                </div>
            ...
            <div class="tf-grid-layout wrapper-shop tf-col-4" id="gridLayout">
                ...
                if(isset($_GET['q']) && !empty($_GET['q'])) {
                    ...
                    $query = "SELECT * FROM product WHERE product_title LIKE '%$searchTerm%' ORDER BY product_title ";
                    
                    // Show message if no search term
                    if(empty(trim($searchTerm))) {
                        echo '<p>Please enter a search term.</p>';
                    } else {
                        $sqsl = mysqli_query($con, $query);
                        
                        if(mysqli_num_rows($sqsl) > 0) {
                ...
                <?php
                            ...
                            echo '<p>No products found matching your search for "' . htmlspecialchars($searchTerm) . '".</p>';
                        }
                    }
                } else {
                    // Default view when no search is performed
                    $sqsl = mysqli_query($con, "SELECT * FROM product ORDER BY RAND(), sno desc Limit 7");
                    while($products = mysqli_fetch_array($sqsl)) {
                        $sql = mysqli_query($con, "SELECT * FROM product_images WHERE product_id='$products[product_id]' ORDER BY id ASC LIMIT 1");
                        $image = mysqli_fetch_array($sql);
                ...
                <div class="loadItem card-product grid" data-availability="In stock" data-brand="Ecomus">
                    <div class="card-product-wrapper">
                        ...
                        <div class="list-product-btn absolute-2">
                            <a 
                                class="xyx-toggle box-icon quickview tf-btn animate-hover-btn radius-3 w-100 justify-content-center d-none d-lg-flex"
                                data-product-id="<?php echo $products['product_id']; ?>">
                                Quick View
                            </a>
                            <a 
